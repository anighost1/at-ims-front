...
import { fetchStorage } from "@/lib/apiCalls/storage";
...
export default function Category() {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['storages', filter, debounceSearch],
                    ...
                    <tbody>
                        {data?.data?.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className={item?.isRefrigerated ? 'text-green-500' : 'text-red-400'}>{item?.isRefrigerated ? 'Yes' : 'No'}</td>
